...
export default ({ navigation }) => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerRight: () => (
        <ClearStorageButton
          onButtonPress={onClearButtonPress}
          disabled={noScannedCodes}
        />
      ),
  ...
  const onClearButtonPress = async () => {
    AsyncStorage.removeItem("ScannedCodes", setScannedCodes([]), (e) =>
      Alert.alert(e)
    );
  };
    ...
    >
      {noScannedCodes ? (
        ...
        <FlatList
          data={scannedCodes}
          renderItem={({ item }) => (
            <ListItem content={item.data} date={item.date} />
          )}
